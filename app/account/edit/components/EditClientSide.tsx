...
const EditClientSide = ({ updateAccount, records }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create an object to send to the updateAccount function
    ...
    try {
      await updateAccount(formDataObject);
    } finally {
      setIsLoading(false);
      router.push("/account");
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="max-w-md mx-auto">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="mt-10 px-3 py-2 rounded-3xl flex items-center bg-gray-500"
          >
            {isLoading && (
              <div role="status">
                <svg
                  aria-hidden="true"
                  className="mr-3 inline w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-white"
                  viewBox="0 0 100 101"
                  fill="none"
